...
if($_POST['guardar']){
//    $i=3;
//    echo $_POST['x_idMiembro'.$i];
//echo $_POST['x_fecha'];
$total_diezmo=0;
for($i=1;$i<$_POST['x_contador'];$i++){
mysql_select_db($database_conexion, $conexion);
$query_miembro = "SELECT idMiembro FROM diezmo WHERE idMiembro='".$_POST['x_idMiembro'.$i]."' AND fecha='".$_POST['x_fecha']."' AND estado=1";
$mostrar_miembro= mysql_query($query_miembro, $conexion) or die(mysql_error());
$total_miembro= mysql_num_rows($mostrar_miembro);    
if($total_miembro>0){
if($_POST['x_diezmo'.$i]>0){
    mysql_select_db($database_conexion, $conexion);
$query_diezmoedit= "UPDATE diezmo SET montoDiezmo='".$_POST['x_diezmo'.$i]."' WHERE idMiembro='".$_POST['x_idMiembro'.$i]."' AND fecha='".$_POST['x_fecha']."' AND estado=1";
mysql_query($query_diezmoedit, $conexion) or die(mysql_error());
$total_diezmo+=$_POST['x_diezmo'.$i];
}else {
mysql_select_db($database_conexion, $conexion);
$query_diezmodelete= "DELETE FROM diezmo WHERE idMiembro='".$_POST['x_idMiembro'.$i]."' AND fecha='".$_POST['x_fecha']."' AND estado=1";
mysql_query($query_diezmodelete, $conexion) or die(mysql_error());
    }
}
else{
if($_POST['x_diezmo'.$i]>0){
mysql_select_db($database_conexion, $conexion);
$query_diezmoadd= "INSERT INTO diezmo VALUES ('','".$_POST['x_idMiembro'.$i]."','".$_POST['x_fecha']."','".$_POST['x_diezmo'.$i]."','','','1')";
mysql_query($query_diezmoadd, $conexion) or die(mysql_error());

$total_diezmo+=$_POST['x_diezmo'.$i];
}
}
}
// reparticion de diezmos a las cuentas


mysql_select_db($database_conexion, $conexion);
$query_cuenta = "SELECT idCuenta, porcentaje FROM cuenta WHERE porcentaje>0 AND estado=1";
...
mysql_select_db($database_conexion, $conexion);
$query_saldo_cuenta = "SELECT idSaldoCuenta FROM saldo_cuenta WHERE idCuenta='".$row_cuenta['idCuenta']."' AND fecha='".$_POST['x_fecha']."' AND diezmo='1' AND estado=1";
$mostrar_saldo_cuenta= mysql_query($query_saldo_cuenta, $conexion) or die(mysql_error());
$total_saldo_cuenta= mysql_num_rows($mostrar_saldo_cuenta);
$ingreso=($row_cuenta['porcentaje']*$total_diezmo)/100;

$fechaCreacion = date("Y-m-d H:i:s");
if($total_saldo_cuenta>0){
if($ingreso>'0'){
mysql_select_db($database_conexion, $conexion);
$query_cuentaedit= "UPDATE  saldo_cuenta SET ingreso='".$ingreso."', fechaModificacion='$fechaCreacion' WHERE idCuenta='".$row_cuenta['idCuenta']."' AND fecha='".$_POST['x_fecha']."' AND diezmo='1' AND estado=1";
mysql_query($query_cuentaedit, $conexion) or die(mysql_error());
}else {
mysql_select_db($database_conexion, $conexion);
$query_cuentadelete= "DELETE FROM saldo_cuenta WHERE idCuenta='".$row_cuenta['idCuenta']."' AND fecha='".$_POST['x_fecha']."' AND diezmo='1' AND estado=1";
mysql_query($query_cuentadelete, $conexion) or die(mysql_error());
}
}else {
$detalle="Apoyo del Diezmo";
mysql_select_db($database_conexion, $conexion);
$query_cuentaadd= "INSERT INTO saldo_cuenta VALUES ('','".$row_cuenta['idCuenta']."','".$_POST['x_fecha']."','','".$detalle."','".$ingreso."','','$fechaCreacion','$fechaCreacion','1','1')";
mysql_query($query_cuentaadd, $conexion) or die(mysql_error());
}
}

//echo $total_diezmo;

Header("Location: diezmolist.php?total=".$total_diezmo);
