...
login.ValidateForm = function(fobj)
{
	...
	if (!ew_HasValue(fobj.password))
		return ew_OnError(this, fobj.password, "Ingrese contrase�a");
...
<table border="0" cellspacing="0" cellpadding="4">
	...
	<tr>
		<td><span class="phpmaker">Contrase�a</span></td>
		<td><span class="phpmaker"><input type="password" name="password" id="password" size="20"></span></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		<td><span class="phpmaker">
		<label><input type="radio" name="rememberme" id="rememberme" value="a"<?php if ($login->sLoginType == "a") { ?> checked="checked"<?php } ?>>Ingresar autom�ticamente hasta que me desconecte expl�citamente</label><br>
		...
		<label><input type="radio" name="rememberme" id="rememberme" value=""<?php if ($login->sLoginType == "") { ?> checked="checked"<?php } ?>>Siempre preguntar por mi usuario y contrase�a</label>
		</span></td>
	</tr>
	<tr>
...
<div align="center">
    <img src="ewmenu/problema.jpg" width="50%" ></img>
</div>
<script language="JavaScript" type="text/javascript">
<!--
...
class clogin {
	...
	function Page_Main() {
		...
		if ($bValidate) {
			...
			if ($bValidate) {
				...
				if (!$bValidPwd)
					$this->setMessage("ID del usuario o contrase�a incorrecta"); // Invalid User ID/password
			} else {
				if ($this->getMessage() == "")
	...
	function ValidateForm($usr, $pwd) {
		...
		if (trim($pwd) == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "Ingrese contrase�a";
		}
